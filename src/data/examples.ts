...
const previewImages = new Map(
  Object.entries(
    import.meta.glob<{ default: ImageMetadata }>("../assets/previews/*.webp"),
  ).map(([key, value]) => [
    key
      .split("/")
      .pop()
...
function toExample(
  ...
  return {
    ...
    previewUrl:
      ref === "latest" && previews.includes(name)
        ? `https://preview.astro.new/${name}`
        : null,
    previewEmbedUrl:
      ref === "latest" && previews.includes(name)
        ? `/latest/preview/${name}`
  ...
  return new Map(
    Object.entries({
      "getting-started": {
        ...
        items: categories["getting-started"].sort(
      ...
      integrations: {
        title: "Integrations",
        slug: "integrations",
        items: categories.integrations.sort(
          sortExamplesByOrder(INTEGRATIONS_ORDER),
        ),
      },
      templates: {
        title: "Templates",
        slug: "templates",
        items: categories.templates.filter(
          (example) => !TEMPLATE_BLOCKLIST.includes(example.name),
        ),
      },
    }),
  );
...
async function getPreviews() {
  const { previews } = await fetch(
    "https://preview.astro.new/metadata.json",
  ).then((res) => res.json());
