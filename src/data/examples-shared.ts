...
export async function getExamples(ref = 'latest') {
	if (ref === 'next') {
		try {
			...
			console.error(`Failed to fetch examples for ref "${ref}", falling back to ref "main" -`, e);
			...
			fetch(githubRequest(astroContentUrl(ref)))
				...
				.then((json) => {
					if (!Array.isArray(json)) {
						throw new Error(`Expected an array of examples, but got: ${JSON.stringify(json)}`);
					}
					return json;
				})
